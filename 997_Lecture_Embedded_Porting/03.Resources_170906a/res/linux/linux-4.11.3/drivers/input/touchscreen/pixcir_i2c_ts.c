			     ...
			     struct pixcir_report_data *report)
{
	...
	if (n == 1) {
		/* 1 point pressed */
		input_report_abs(ts->input, ABS_X, report->pos[0].x);
		input_report_abs(ts->input, ABS_Y, report->pos[0].y);
		...
		input_report_abs(ts->input, ABS_X, report->pos[1].x);
		input_report_abs(ts->input, ABS_Y, report->pos[1].y);
		input_report_key(ts->input, BTN_TOUCH, 1);
		input_report_abs(ts->input, ABS_PRESSURE, 1);
		input_sync(ts->input);
	}
